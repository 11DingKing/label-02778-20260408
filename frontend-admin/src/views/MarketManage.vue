<template>
  <div>
    ...
    <el-card shadow="never" class="table-card">
      <el-table :data="list" v-loading="loading" stripe :empty-text="'暂无市场数据'" style="width:100%">
        ...
        <el-table-column label="分类" width="110" align="center">
          <template #default="{ row }">
            <el-tag :type="categoryType(row.category)" effect="light" round>{{ categoryName(row.category) }}</el-tag>
          </template>
        </el-table-column>
...
const categoryTypes = ['warning', 'info', '']
function categoryName(c) { return categoryNames[c] || '其他' }
function categoryType(c) { return categoryTypes[c] || '' }

async function loadData() {
  loading.value = true
  try {
    const res = await adminApi.market({ ...query.value, keyword: keyword.value })
    list.value = res.list
