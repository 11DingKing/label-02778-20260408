...
use crate::middleware::{extract_claims, check_rate_limit};
use crate::models::*;
use crate::services;
use crate::AppState;
use super::get_ip;
...
pub async fn register(req: HttpRequest, state: web::Data<AppState>, body: web::Json<RegisterReq>) -> Result<HttpResponse, AppError> {
    ...
    check_rate_limit(&req, &state, None)?;
...
pub async fn me(req: HttpRequest, state: web::Data<AppState>) -> Result<HttpResponse, AppError> {
    ...
    check_rate_limit(&req, &state, Some(&claims))?;
...
pub async fn refresh(req: HttpRequest, state: web::Data<AppState>, body: web::Json<RefreshTokenReq>) -> Result<HttpResponse, AppError> {
    body.validate().map_err(|e| AppError::BadRequest(e.to_string()))?;
    check_rate_limit(&req, &state, None)?;
